...
// sanitize text for filename (keep CJK letters, Latin letters/numbers, space/underscore/hyphen)
function safeName(s){
...
export default async (request) => {
  try {
    ...
    const base  = [title, date, slug].filter(Boolean).join('-');
    ...
    return new Response(content, {
      ...
      headers: {
        ...
        'content-disposition': `attachment; filename="${base || slug}.zip"`
      }
    });
  } catch (e) {
